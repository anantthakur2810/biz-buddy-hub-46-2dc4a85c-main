...
export function BusinessAssistant() {
    ...
    return (<div className="stat-card animate-fade-in">
      ...
      <div className="mb-4 flex flex-wrap gap-2">
        {assistantRecommendations.map((item) => (<button key={item.id} type="button" onClick={() => setActiveId(item.id)} className={`rounded-full border px-3 py-2 text-sm transition-colors ${activeId === item.id
      ...
      {activeRecommendation && (<div className="rounded-xl border border-border bg-muted/40 p-4">
