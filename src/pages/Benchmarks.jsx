...
import { annualBusinessSummary, benchmarkCategories, benchmarkCategoryDatasets, benchmarkCategorySummaries, currentBusinessBenchmarkCategory, currentBusinessBenchmarkCategoryProfile, mockDashboardStats, yearlyBusinessDataset, } from '@/data/mockData';
...
export default function Benchmarks() {
    const [selectedCategory, setSelectedCategory] = useState(currentBusinessBenchmarkCategory);
    const selectedCategoryProfile = useMemo(() => benchmarkCategories.find((category) => category.key === selectedCategory) ??
        currentBusinessBenchmarkCategoryProfile, [selectedCategory]);
    ...
    const comparisons = [
        { label: 'Annual Revenue', yours: yourBusiness.revenue, avg: selectedSummary.totalRevenue, format: (value) => formatCurrency(value) },
        { label: 'Customers', yours: yourBusiness.customers, avg: selectedSummary.totalCustomers, format: (value) => value.toString() },
        { label: 'Products Listed', yours: yourBusiness.products, avg: selectedSummary.totalProducts, format: (value) => value.toString() },
        { label: 'Completed Orders', yours: yourBusiness.salesCount, avg: selectedSummary.totalOrders, format: (value) => value.toString() },
        { label: 'Avg Order Value', yours: yourBusiness.averageOrderValue, avg: selectedSummary.averageOrderValue, format: (value) => formatCurrency(value) },
        { label: 'Growth Rate', yours: yourBusiness.revenueGrowth, avg: selectedSummary.revenueGrowth, format: (value) => `${value}%` },
    ];
    const categories = [
        {
    ...
    return (<div className="space-y-6">
      ...
      <div className="grid gap-6 lg:grid-cols-[1.2fr_0.8fr]">
          ...
          <div className="grid gap-4 sm:grid-cols-[220px_1fr]">
            <div className="space-y-2">
              ...
              <Select value={selectedCategory} onValueChange={(value) => setSelectedCategory(value)}>
                ...
                <SelectContent>
                  {benchmarkCategories.map((category) => (<SelectItem key={category.key} value={category.key}>
                      {category.label}
                    </SelectItem>))}
                </SelectContent>
              </Select>
        ...
        <div className="stat-card animate-fade-in">
          ...
          <div className="space-y-3">
            {excludedCategories.map((category) => (<div key={category} className="rounded-lg border border-border bg-muted/30 px-4 py-3 text-sm text-foreground">
                {category}
              </div>))}
          </div>
        </div>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <div className="stat-card animate-fade-in">
          ...
          <div className="space-y-4">
            {percentiles.map((percentile) => (<div key={percentile.label} className="flex items-center gap-4">
                <span className="w-28 shrink-0 text-sm font-medium text-foreground">{percentile.label}</span>
                <div className="relative h-2 flex-1 rounded-full bg-muted">
                  <div className="absolute top-1/2 h-3.5 w-3.5 -translate-y-1/2 rounded-full border-2 border-card bg-primary transition-all duration-1000" style={{ left: `${percentile.value}%` }}/>
                </div>
        ...
        <div className="stat-card animate-fade-in">
          ...
          <div className="space-y-4">
            ...
            return (<div key={comparison.label}>
                  ...
                  <div className="relative h-2.5 overflow-hidden rounded-full bg-muted">
                    <div className="absolute inset-y-0 left-0 rounded-full bg-muted-foreground/30" style={{ width: `${(comparison.avg / maxValue) * 100}%` }}/>
                    <div className="absolute inset-y-0 left-0 rounded-full bg-primary" style={{ width: `${(comparison.yours / maxValue) * 100}%` }}/>
                  </div>
                </div>);
        })}
          </div>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <div className="stat-card animate-fade-in">
          ...
          <div className="divide-y divide-border">
            {categories.map((category) => (<div key={category.name} className="flex items-center justify-between py-3">
                <div>
        ...
        <div className="stat-card animate-fade-in">
          ...
          <div className="space-y-3">
            {insights.map((insight, index) => (<div key={index} className="flex items-start gap-3 rounded-xl bg-muted p-3">
      ...
      <div className="stat-card animate-fade-in">
        ...
        <div className="overflow-x-auto">
          <div className="min-w-[760px]">
            ...
            {selectedDataset.map((month) => (<div key={month.name} className="grid grid-cols-[90px_1.3fr_0.9fr_1fr_1.1fr] border-b border-border px-4 py-3 text-sm last:border-b-0">
      ...
      <div className="stat-card animate-fade-in">
        ...
        <div className="flex h-44 items-end gap-4">
          {recentSixMonths.map((month, index) => (<div key={month.name} className="flex flex-1 flex-col items-center gap-1">
              <div className="flex h-36 items-end gap-1">
                <div className="w-5 rounded-t bg-primary transition-all duration-700" style={{ height: `${(yourRevenueTrend[index] / maxRevenue) * 140}px` }}/>
                <div className="w-5 rounded-t bg-muted-foreground/30 transition-all duration-700" style={{ height: `${(peerRevenueTrend[index] / maxRevenue) * 140}px` }}/>
              </div>
