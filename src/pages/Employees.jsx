...
export default function Employees() {
    ...
    return (<div className="space-y-6">
                    ...
                    <TableCell colSpan={5} className="py-10 text-center text-muted-foreground">
                      ...
                      <TableCell className="text-right">
                        <div className="flex justify-end gap-2">
                          <Button variant="outline" size="sm" className="gap-2" onClick={() => openEditDialog(employee)}>
                            <Pencil className="h-4 w-4"/>
                            Edit
                          </Button>
                          <Button variant="outline" size="sm" className="gap-2 text-destructive hover:text-destructive" onClick={() => setEmployeeToDelete(employee)}>
        ...
        <DialogContent className="sm:max-w-[500px]">
          ...
          <div className="grid gap-4 py-4">
            <div className="grid gap-2">
              ...
              <Input id="create-employee-name" value={createForm.name} onChange={(event) => setCreateForm((current) => ({ ...current, name: event.target.value }))} placeholder="Riya Sharma"/>
            </div>
            <div className="grid gap-2 sm:grid-cols-2">
              <div className="grid gap-2">
                <Label htmlFor="create-employee-phone">Phone Number</Label>
                <Input id="create-employee-phone" value={createForm.phone} onChange={(event) => setCreateForm((current) => ({ ...current, phone: event.target.value }))} placeholder="9876500010"/>
              </div>
              <div className="grid gap-2">
                <Label htmlFor="create-employee-password">Password</Label>
                <Input id="create-employee-password" type="password" value={createForm.password} onChange={(event) => setCreateForm((current) => ({ ...current, password: event.target.value }))} placeholder="Emp@123"/>
              </div>
            </div>
            <div className="grid gap-2">
              <Label htmlFor="create-employee-title">Job Title</Label>
              <Input id="create-employee-title" value={createForm.title} onChange={(event) => setCreateForm((current) => ({ ...current, title: event.target.value }))} placeholder="Cashier"/>
            </div>
          </div>
          <DialogFooter>
        ...
        <DialogContent className="sm:max-w-[500px]">
          ...
          <div className="grid gap-4 py-4">
            <div className="grid gap-2">
              ...
              <Input id="edit-employee-name" value={editForm.name} onChange={(event) => setEditForm((current) => ({ ...current, name: event.target.value }))}/>
            </div>
            <div className="grid gap-2 sm:grid-cols-2">
              <div className="grid gap-2">
                <Label htmlFor="edit-employee-phone">Phone Number</Label>
                <Input id="edit-employee-phone" value={editForm.phone} onChange={(event) => setEditForm((current) => ({ ...current, phone: event.target.value }))}/>
              </div>
              <div className="grid gap-2">
                <Label htmlFor="edit-employee-password">Password</Label>
                <Input id="edit-employee-password" type="password" value={editForm.password} onChange={(event) => setEditForm((current) => ({ ...current, password: event.target.value }))}/>
              </div>
            </div>
            <div className="grid gap-2">
              <Label htmlFor="edit-employee-title">Job Title</Label>
              <Input id="edit-employee-title" value={editForm.title} onChange={(event) => setEditForm((current) => ({ ...current, title: event.target.value }))}/>
            </div>
          </div>
          <DialogFooter>
      ...
      <AlertDialog open={Boolean(employeeToDelete)} onOpenChange={(open) => {
            if (!open) {
                setEmployeeToDelete(null);
            }
        }}>
