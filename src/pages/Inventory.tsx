...
export default function Inventory() {
  ...
  const [stockAdjustment, setStockAdjustment] = useState({ type: 'add' as 'add' | 'subtract', amount: 0 });
              ...
              return (
                <TableRow key={product.id} className="table-row-hover">
                  ...
                  <TableCell className="text-right">
                    <span className={cn(
                      'font-medium',
                      product.quantity <= product.minStock && 'text-warning',
      ...
      <Dialog open={isAdjustStockDialogOpen && canManageInventory} onOpenChange={setIsAdjustStockDialogOpen}>
        <DialogContent className="sm:max-w-[400px]">
          <DialogHeader>
            ...
            <DialogDescription>
              {selectedProduct && (
                <>Adjust stock for <strong>{selectedProduct.name}</strong>. Current quantity: <strong>{selectedProduct.quantity}</strong></>
              )}
            </DialogDescription>
          </DialogHeader>
          <div className="grid gap-4 py-4">
            ...
            <div className="grid gap-2">
              ...
              <Input
                ...
                onChange={(e) => setStockAdjustment({ ...stockAdjustment, amount: parseInt(e.target.value) || 0 })}
                placeholder="Enter quantity"
              />
            </div>
            {selectedProduct && stockAdjustment.amount > 0 && (
