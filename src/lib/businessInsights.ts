...
const productLookup = mockProducts.reduce<Record<string, (typeof mockProducts)[number]>>((lookup, product) => {
  lookup[product.id] = product;
  return lookup;
}, {});

const customerOrderCounts = completedSales.reduce<Record<string, number>>((lookup, sale) => {
  lookup[sale.customerId] = (lookup[sale.customerId] ?? 0) + 1;
  return lookup;
}, {});

const monthlyProfitDataset = yearlyBusinessDataset.map((month, monthIndex) => {
  const monthlyCompletedSales = completedSales.filter(
    (sale) =>
      sale.createdAt.getFullYear() === annualBusinessSummary.year &&
      sale.createdAt.getMonth() === monthIndex,
  );

  const profit = roundMetric(
    monthlyCompletedSales.reduce((sum, sale) => {
...
const productPerformance = mockProducts
  .map((product) => {
    const relevantItems = completedSales.flatMap((sale) =>
      sale.items
...
const categoryPerformance = Object.values(
  productPerformance.reduce<Record<string, { name: string; revenue: number; profit: number; unitsSold: number }>>((lookup, entry) => {
...
export const analyticsHighlights = {
  ...
  repeatCustomerRate: roundMetric(
