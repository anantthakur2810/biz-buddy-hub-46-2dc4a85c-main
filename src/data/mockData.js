...
const customerLookup = customerDirectory.reduce((lookup, customer) => {
...
const buildSaleItems = (seedItems) => seedItems.map(({ productId, quantity }) => {
...
export const mockSales = saleSeeds
    .map((seed) => {
...
const purchasesByCustomer = completedSales.reduce((lookup, sale) => {
    lookup[sale.customerId] = roundCurrency((lookup[sale.customerId] ?? 0) + sale.total);
    return lookup;
}, {});
export const mockCustomers = customerDirectory
    .map((customer) => ({
    ...customer,
...
export const yearlyBusinessDataset = MONTH_NAMES.map((name, monthIndex) => {
    const monthlySales = completedSales.filter((sale) => sale.createdAt.getFullYear() === SAMPLE_YEAR &&
        sale.createdAt.getMonth() === monthIndex);
    const newCustomers = mockCustomers.filter((customer) => customer.createdAt.getFullYear() === SAMPLE_YEAR &&
        customer.createdAt.getMonth() === monthIndex).length;
    return {
        name,
...
const buildBenchmarkMonths = (months) => months.map((month) => ({
    ...month,
    averageOrderValue: roundCurrency(month.revenue / month.orders),
}));
const benchmarkCategoryDefinitions = [
    {
...
export const benchmarkCategories = benchmarkCategoryDefinitions.map(({ key, label, description, peerCount, totalCustomers, totalProducts }) => ({
    key,
    label,
    description,
    peerCount,
...
export const currentBusinessBenchmarkCategory = 'consumer-electronics';
export const currentBusinessBenchmarkCategoryProfile = benchmarkCategories.find((category) => category.key === currentBusinessBenchmarkCategory) ?? benchmarkCategories[0];
export const benchmarkCategoryDatasets = benchmarkCategoryDefinitions.reduce((lookup, definition) => {
    lookup[definition.key] = buildBenchmarkMonths(definition.months);
    return lookup;
}, {});
export const benchmarkCategorySummaries = benchmarkCategoryDefinitions.reduce((lookup, definition) => {
...
export const categoryData = Object.entries(mockProducts.reduce((lookup, product) => {
    lookup[product.category] = (lookup[product.category] ?? 0) + 1;
    return lookup;
}, {})).map(([name, value]) => ({ name, value }));
const totalAnnualRevenue = roundCurrency(yearlyBusinessDataset.reduce((sum, month) => sum + month.revenue, 0));
const firstHalfRevenue = roundCurrency(yearlyBusinessDataset.slice(0, 6).reduce((sum, month) => sum + month.revenue, 0));
const secondHalfRevenue = roundCurrency(yearlyBusinessDataset.slice(6).reduce((sum, month) => sum + month.revenue, 0));
const benchmarkFirstHalfRevenue = roundCurrency(benchmarkCompetitionDataset.slice(0, 6).reduce((sum, month) => sum + month.revenue, 0));
const benchmarkSecondHalfRevenue = roundCurrency(benchmarkCompetitionDataset.slice(6).reduce((sum, month) => sum + month.revenue, 0));
const customerBaseByMonth = MONTH_NAMES.map((_, monthIndex) => mockCustomers.filter((customer) => customer.createdAt.getFullYear() < SAMPLE_YEAR ||
    (customer.createdAt.getFullYear() === SAMPLE_YEAR && customer.createdAt.getMonth() <= monthIndex)).length);
